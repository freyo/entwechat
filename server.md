...
回复的消息可以为 `null`，此时 SDK 会返回给微信一个 "SUCCESS"，你也可以回复一个普通字符串，比如：`欢迎关注`，此时 SDK 会对它进行一个封装，产生一个 [`EntWeChat\Message\Text`](https://github.com/EntWeChat/message/blob/master/src/Text.php) 类型的消息并在最后的 `$server->serve();` 时生成对应的消息 XML 格式。

如果你想返回一个自己手动拼的原生 XML 格式消息，请返回一个 [`EntWeChat\Message\Raw`](https://github.com/EntWeChat/message/blob/master/src/Raw.php) 实例即可。
